...
| P/S | 18/26 |
| Cash P/E | 17/26 |
| Shareholder Yield（含明确零值） | 22/26 |

Cash P/E 的其余状态：

- `N/M`：Fluid（首版已知直接损失使 Protocol Earnings 小于零）。
- `N/A`：BGB、BNB、OKB、MNT（缺少同一经济主体的可比收入与成本边界，不强行套传统 P/E）。
- `待核实`：EigenLayer、HYPE、JustLend、Morpho。

Shareholder Yield 的 22 个可用值中，13 个为正值候选，9 个为已识别的零值；Aave、BGB、HYPE、JustLend 仍待核实。
...
1. 链上一手数据：官方节点、RPC/API 原始账本、合约事件、协议地址和可验证资金流。
2. 项目官方披露：公告、报告、治理执行记录、官方仪表盘和审计材料。
3. 第三方兜底：DefiLlama、Token Terminal、Dune 等只作补充或交叉检查；必须标记为估算，不得覆盖链上或官方证据。

来源冲突时不取平均。链上时点余额、累计持仓成本和部分 API 窗口不能冒充 TTM；完整年度账本未闭合时保持“待核实”。

## 首版估算规则

- 所有指标均按“链上优先、官方第二、第三方最后”的顺序选择主值。
- 第三方聚合值只有在一手与官方数据不可得时才能作为候选，并必须显示 `~`。
- Revenue 有可用值、但必要直接成本或已实现损失尚未单列时，首版以 Revenue 减去已知成本和损失计算 Protocol Earnings；未单列的增量成本暂按零估算，并以 `~` 和低置信度标记。
- 短于 TTM 的已执行资本回报只能显示已观察窗口金额，不得直接年化或标记为完整 TTM。
...
- HYPE：旧 `$792,146,570` 收入代理值及 `9.30x` 已撤回。官方链上成交窗口不足一年，因此 Revenue、Protocol Earnings、P/S、Cash P/E 和回购收益率均保持“待核实”。
...
- HYPE 已改为官方链上诊断：Assistance Fund 地址、HYPE 余额和部分成交可验证，但官方 fills API 的可见窗口不足 TTM，不能生成年度 P/E 或回购收益率。
- GMX、Aster 等执行金额较大的资本回报仍需按新的链上优先规则继续复核。
